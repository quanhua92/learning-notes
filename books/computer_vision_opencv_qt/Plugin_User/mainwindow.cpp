    ...
    foreach(QFileInfo filter, filters){
        if(QLibrary::isLibrary(filter.absoluteFilePath())){
            ...
            if(dynamic_cast<CvPluginInterface*>(pluginLoader.instance())){
                ...
                QMessageBox::warning(
                            this, tr("Warning"),
                            QString(tr("Make sure %1 is a correct plugin<br>"
                                       "and it's not in use by some other apps!")).arg(filter.fileName()));
            }
        } else {
            QMessageBox::warning(
                        this, tr("Warning"),
                        QString(tr("Make sure only plugins exist in plugins folder. <br> %1 is not a plugin"
                                   )).arg(filter.fileName()));
        }
    }
        ...
        if(plugin)
        {
            if(QFile::exists(ui->inputImgEdit->text()))
            {
                ...
                inputImage = imread(ui->inputImgEdit->
                                    text().toStdString()); plugin->processImage(inputImage, outputImage);
